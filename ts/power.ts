export class Power {
    ...
    private _shutdownButton: HTMLButtonElement | undefined;
    private _restartButton: HTMLButtonElement | undefined;
    private _hibernateButton: HTMLButtonElement | undefined;
    private _suspendButton: HTMLButtonElement | undefined;
    private _control: HTMLDivElement | undefined;

    private _canShutdown: boolean  = false;
    private _canRestart: boolean   = false;
    private _canHibernate: boolean = false;
    ...
    private _initialiseListeners () {
        ...
        this._shutdownButton.addEventListener ('click', async () => {
            if (!window.lightdm || !window.lightdm.can_shutdown) return;
            await window.wait (1000);
            window.lightdm.shutdown();
        });
        this._restartButton.addEventListener ('click', async () => {
            if (!window.lightdm || !window.lightdm.can_restart) return;
            await window.wait (1000);
        ...
        this._suspendButton.addEventListener ('click', async () => {
            if (!window.lightdm || !window.lightdm.can_suspend) return;
            await window.wait (1000);
